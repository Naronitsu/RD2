...
Data collection is performed by instrumenting the simulated ECU replay service to record, for each run, full-trace elapsed time, process CPU time, heap-delta indicators, throughput, and violation flag totals with RV enabled versus disabled. The replay traces are derived from the Kaggle Car ECU Datalogs dataset~\cite{gxkok21ecudatalogs}. Faults are injected into the abnormal dataset to evaluate whether the monitor reports the expected violation classes.

The collected data are analysed using descriptive statistics (mean, standard deviation, and tail percentiles) and direct comparisons between monitored and unmonitored runs. This quantifies both runtime cost and detection behaviour against the study objectives.
